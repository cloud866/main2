public class Parallelogram extends Quadrangle implements Figure {
    ...
    @Override
    public double area() {
        return Math.max(a, b) * getHeight();  // areas: big side * small height
    }
    ...
    @Override
    public double getLargeDiagonal() {  // search diagonals bu cosine theorem
    ...
    public double getHeight() {
        return a * Math.min(a * Math.sin(Math.toRadians(alpha)), b * Math.sin(Math.toRadians(beta)));  // search height using sin
    }
